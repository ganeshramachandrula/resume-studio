...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const translated = getTranslatedBlogPosts('pt')
...
export default async function PortugueseBlogPostPage({ params }: Props) {
  const { slug } = await params
  ...
  return (
    <article className="pt-32 pb-20 px-4">
      <div className="max-w-3xl mx-auto">
        <Link href="/pt/blog" className="inline-flex items-center gap-1 text-gray-400 hover:text-white text-sm mb-8 transition-colors">
        ...
        <header className="mb-10">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-500">
            ...
            <span>{new Date(meta.publishedAt).toLocaleDateString('pt', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
          </div>
        </header>
