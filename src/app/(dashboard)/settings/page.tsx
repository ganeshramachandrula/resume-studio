...
import { Crown, CreditCard, Loader2, LogOut, Zap } from 'lucide-react'
import { ReferralSection } from '@/components/referral-section'

export default function SettingsPage() {
  const router = useRouter()
  const { profile, setProfile } = useAppStore()
  const { usageCount, limit, isPaid, credits } = useUsage(profile)
  ...
  const planLabel = profile?.plan === 'pro' ? 'Pro' : profile?.plan === 'basic' ? 'Basic' : 'Free Plan'
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {usageCount}/{limit} documents used this month
                  {credits > 0 && ` + ${credits} credit${credits !== 1 ? 's' : ''}`}
                </p>
              </div>
            </div>
            <Badge variant={isPaid ? 'accent' : 'secondary'}>
              {isPaid ? 'Active' : 'Free'}
            </Badge>
          </div>
