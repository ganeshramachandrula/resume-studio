...
import {
  GENERATE_FOLLOW_UP_EMAIL_SYSTEM,
  buildFollowUpEmailPrompt,
} from '@/lib/ai/prompts/generate-follow-up-email'
import { mockFollowUpEmailData } from '@/lib/ai/mock-responses'
...
import { validateBody, isValidationError, followUpEmailSchema } from '@/lib/security/validation'
...
export async function POST(request: Request) {
  try {
    ...
    if (!ipRl.allowed) {
      logSecurityEvent('rate_limit_hit', request, undefined, { route: 'generate-follow-up-email' })
    ...
    if (!userRl.allowed) {
      logSecurityEvent('rate_limit_hit', request, user.id, { route: 'generate-follow-up-email' })
      return rateLimitResponse(userRl.retryAfterSeconds!)
    }

    // Email verification gate
    if (!isEmailVerified(user)) {
      logSecurityEvent('email_not_verified', request, user.id, { route: 'generate-follow-up-email' })
    ...
    // Validate input
    const body = await validateBody(request, followUpEmailSchema)
    if (isValidationError(body)) {
      logSecurityEvent('validation_error', request, user.id, { route: 'generate-follow-up-email' })
      return body
    }

    const { parsedJD, experience, jobDescriptionId, contactInfo, language, interviewNotes } = body
    ...
    if (usageResult && !usageResult.allowed) {
      logSecurityEvent('usage_limit_hit', request, user.id, { route: 'generate-follow-up-email' })
      return NextResponse.json(
    ...
    logSecurityEvent('generation_attempt', request, user.id, { route: 'generate-follow-up-email' })
    ...
    const followUpEmail = isAIConfigured()
      ? await generateJSONWithClaude(
          GENERATE_FOLLOW_UP_EMAIL_SYSTEM,
          buildFollowUpEmailPrompt(parsedJD, experience, interviewNotes, contactInfo, effectiveLanguage)
        )
      : mockFollowUpEmailData

    if (!isPro && usageResult?.reason !== 'credit_used') {
      return NextResponse.json({ success: true, content: followUpEmail, saved: false })
    }
    ...
    const { data: saveResult, error } = await supabase.rpc('save_document_with_bundle_tracking', {
      ...
      p_type: 'follow_up_email',
      p_title: `Follow-Up Email - ${parsedJD.role_title || 'Untitled'} at ${parsedJD.company_name || 'Company'}`,
      p_content: followUpEmail as Record<string, unknown>,
    })

    if (error) throw error

    return NextResponse.json({ success: true, data: saveResult, content: followUpEmail, saved: true })
  } catch (error: unknown) {
    return safeErrorResponse(error, 'generate-follow-up-email')
  }
}
