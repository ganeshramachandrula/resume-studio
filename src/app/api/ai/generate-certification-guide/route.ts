...
import {
  ...
  buildCertificationGuidePrompt,
} from '@/lib/ai/prompts/generate-certification-guide'
import { mockCertificationGuideData } from '@/lib/ai/mock-responses'
...
export async function POST(request: Request) {
  try {
    ...
    if (!ipRl.allowed) {
      logSecurityEvent('rate_limit_hit', request, undefined, { route: 'generate-certification-guide' })
    ...
    if (!userRl.allowed) {
      logSecurityEvent('rate_limit_hit', request, user.id, { route: 'generate-certification-guide' })
      return rateLimitResponse(userRl.retryAfterSeconds!)
    }

    // Email verification gate
    if (!isEmailVerified(user)) {
      logSecurityEvent('email_not_verified', request, user.id, { route: 'generate-certification-guide' })
    ...
    if (isValidationError(body)) {
      logSecurityEvent('validation_error', request, user.id, { route: 'generate-certification-guide' })
      return body
    }
    ...
    if (usageResult && !usageResult.allowed) {
      logSecurityEvent('usage_limit_hit', request, user.id, { route: 'generate-certification-guide' })
      return NextResponse.json(
    ...
    logSecurityEvent('generation_attempt', request, user.id, { route: 'generate-certification-guide' })
    ...
    const certificationGuide = isAIConfigured()
      ? await generateJSONWithClaude(GENERATE_CERTIFICATION_GUIDE_SYSTEM, buildCertificationGuidePrompt(parsedJD, experience, contactInfo, effectiveLanguage), 4096)
      : mockCertificationGuideData

    if (!isPro) {
      return NextResponse.json({ success: true, content: certificationGuide, saved: false })
    }
    ...
    const { data: saveResult, error } = await supabase.rpc('save_document_with_bundle_tracking', {
      ...
      p_type: 'certification_guide',
      p_title: `Certification Guide - ${parsedJD.role_title || 'Untitled'} at ${parsedJD.company_name || 'Company'}`,
      p_content: certificationGuide as Record<string, unknown>,
    })

    if (error) throw error

    return NextResponse.json({ success: true, data: saveResult, content: certificationGuide, saved: true })
  } catch (error: unknown) {
    return safeErrorResponse(error, 'generate-certification-guide')
  }
}
