...
export async function POST(request: Request) {
  try {
    ...
    if (!ipRl.allowed) {
      logSecurityEvent('rate_limit_hit', request, undefined, { route: 'generate-cover-letter' })
    ...
    if (!userRl.allowed) {
      logSecurityEvent('rate_limit_hit', request, user.id, { route: 'generate-cover-letter' })
      return rateLimitResponse(userRl.retryAfterSeconds!)
    }

    // Email verification gate
    if (!isEmailVerified(user)) {
      logSecurityEvent('email_not_verified', request, user.id, { route: 'generate-cover-letter' })
    ...
    if (isValidationError(body)) {
      logSecurityEvent('validation_error', request, user.id, { route: 'generate-cover-letter' })
      return body
    }
    ...
    if (usageResult && !usageResult.allowed) {
      logSecurityEvent('usage_limit_hit', request, user.id, { route: 'generate-cover-letter' })
      return NextResponse.json(
    ...
    logSecurityEvent('generation_attempt', request, user.id, { route: 'generate-cover-letter' })

    const coverLetter = isAIConfigured()
      ? await generateJSONWithClaude(GENERATE_COVER_LETTER_SYSTEM, buildCoverLetterPrompt(parsedJD, experience, contactInfo))
      : mockCoverLetterData
